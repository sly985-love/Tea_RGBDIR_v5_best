...
class Colors:
    ...
    def __init__(self):
        ...
        hexs = ("3080BD", 'FF9D97', 'FF701F', 'FFB21D', 'CFD231', '48F90A', '92CC17', '3DDB86', '1A9334', '00D4BB',
...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font='Arial.ttf', pil=False, example='abc'):
        ...
        if self.pil:  # use PIL
            ...
            #                            size=font_size or max(round(sum(self.im.size) / 5 * 0.035), 1))
            self.font = check_pil_font(font='Arial.Unicode.ttf' if non_ascii else font,
                                       size=font_size or max(round(sum(self.im.size) / 2 * 0.035), 12))
        else:  # use cv2
            self.im = im
        self.lw = line_width or max(round(sum(im.shape) / 2 * 0.003), 2)  # line width
        # self.lw = line_width or max(round(sum(im.shape) / 5 * 0.003), 1)  # line width
...
def plot_labels(labels, names=(), save_dir=Path('')):
    ...
    # color_codes = True
    # sn.set_theme(font='Times New Roman', color_codes=True)
    sn.pairplot(x, corner=True, diag_kind='auto', kind='hist', diag_kws=dict(bins=50), plot_kws=dict(pmax=0.9))
    plt.savefig(save_dir / 'labels_correlogram.jpg', dpi=800)
    plt.close()
    ...
    plt.savefig(save_dir / 'labels.jpg', dpi=800)
    matplotlib.use('Agg')
    plt.close()
