...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.norm_sly = nn.LayerNorm(in_features)
    ...
    def forward(self, x):
        ...
        x = self.fc2(x)
        x = self.drop(x)
        x = x.view(-1, C_sly, H_sly, W_sly)
        return x


class MHSA(nn.Module):
    def __init__(self, dim, num_heads=16, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        # print("head_dim,num_heads,qk_scale",head_dim,num_heads,qk_scale)
        ...
        self.norm_sly = nn.LayerNorm(dim)
...
class C3shunted2(nn.Module):
    # class BoT3(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=2, e=0.5, num_heads=16, mlp_ratio=4, qkv_bias=True, qk_scale=None, drop=0.,
                 attn_drop=0., drop_path=0., act_layer=nn.GELU, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 sr_ratio=1):  # ch_in, ch_out, number, , expansion,w,h
        super(C3shunted2, self).__init__()
        # print("C3shunted2 c2 dim,num_heads", c2, num_heads)
        c_ = c2 // 2  # hidden channels
        # print("C3shunted2 c2 dim,num_heads", c_, num_heads)
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # act=FReLU(c2)
        self.m = nn.Sequential(
            *[BottleneckTransformer(dim=c_, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                                    qk_scale=qk_scale, drop=drop, attn_drop=attn_drop, drop_path=drop_path,
                                    act_layer=act_layer, norm_layer=norm_layer, sr_ratio=sr_ratio) for _ in
              range(n)])
